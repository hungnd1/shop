...
class OldMoListenerController extends Controller
{
    ...
    {
        if (empty($msisdn)) return $this->responseError('MSISDN Empty');
        if (empty($mocontent)) return $this->responseError('Message Empty');
...
//        $servicenumber = $tmp;
        ...
        if (empty($msisdn)) {
            return $this->responseError(Message::MSG_KHONG_NHAN_DIEN_THUE_BAO);
        }
