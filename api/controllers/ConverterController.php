...
class ConverterController extends Controller{
    ...
    public function actionUpdateStatusContent(){
        ...
        if($status == null || !isset(ContentProfile::$stream_status[$status])){
            return [
                'error'=>2,
                'message'=>Yii::t('app','tham_so_khong_hop_le')
            ];
        }
        ...
        if (!$contentProfile) {
            return [
                'error'=>2,
                'message'=>Yii::t('app','profile_notfound')
            ];
        }
    ...
    public function actionContentConverted()
    {
        ...
        if ($profile_id == null) {
            throw new InvalidValueException($this->replaceParam(Message::MSG_NULL_VALUE, ['profile_id']));
        }
        if ($content_id == null) {
            throw new InvalidValueException($this->replaceParam(Message::MSG_NULL_VALUE, ['content_id']));
        }
        if ($url == null) {
            throw new InvalidValueException($this->replaceParam(Message::MSG_NULL_VALUE, ['url']));
        }
        if ($bitrate == null) {
            throw new InvalidValueException($this->replaceParam(Message::MSG_NULL_VALUE, ['bitrate']));
        }
        if ($width == null) {
            throw new InvalidValueException($this->replaceParam(Message::MSG_NULL_VALUE, ['width']));
        }
        if ($height == null) {
            throw new InvalidValueException($this->replaceParam(Message::MSG_NULL_VALUE, ['height']));
        }
        ...
        if (!$contentProfile) {
            throw new InvalidValueException(Message::CONTENT_PROFILE_NOT_FOUND);
        }
        ...
        return ['message'=>Message::CONTENT_PROFILE_UPDATE_CONVERTED_SUCCESS];
    }
