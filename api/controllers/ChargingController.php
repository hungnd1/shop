...
class ChargingController extends Controller
{
    ...
    public function actionBuyContent($username, $content_id, $promotion = 0, $promotion_note = '')
    {
        ...
        if (!$this->subscriber) {
            return $this->responseError('Khong tim thay thue bao');
        }

        if ($this->subscriber->status != Subscriber::STATUS_ACTIVE){
            return $this->responseError('Thue bao chua kich hoat');
        }

        $content = Content::findOne($content_id);
        if (!   $content) {
            return $this->responseError('Noi dung muon mua khong co');
        }
