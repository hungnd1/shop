...
class StreamingServer extends \yii\db\ActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            [['port'], 'integer', 'min' => 1, 'message' => '{attribute} phải là số nguyên dương', 'tooSmall' => '{attribute} phải là số nguyên dương'],
            [['name'], 'string', 'max' => 200],
            [['ip', 'host'], 'string', 'max' => 64],
            [['content_status_api', 'content_api'], 'string', 'max' => 255],
            ['host', 'match', 'pattern' => '/^((?=[a-z0-9-]{1,63}\.)(xn--)?[a-z0-9]+(-[a-z0-9]+)*\.)+[a-z]{2,63}$/', 'message' => 'Tên miền không đúng định dạng'],
            ['ip', 'match', 'pattern' => '/^(((?:25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[1-9][0-9]|[0-9])(?:\\.(?:25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[1-9][0-9]|[0-9])){3})|((?:(?:[0-9a-fA-F:]){1,4}(?:(?::(?:[0-9a-fA-F]){1,4}|:)){2,7})+))$/', 'message' => '{attribute} không đúng định dạng'],
            ['ip', 'validateUnique']
        ];
    }
    ...
    public function validateUnique($attribute, $params)
    {
        if (!$this->hasErrors()) {
            ...
            if ($streamingServer && $streamingServer->id != $this->id) {
                $this->addError($attribute, "$attribute đã tồn tại. Vui lòng nhập $attribute khác!");
            }
        }
    }
    ...
    public function softDelete()
    {
        if ($this->status != self::STATUS_INACTIVE) {
            $res['success'] = false;
            $res['message'] = 'Bạn chỉ được phép xóa các địa chỉ phân phối nội dung có trạng thái "Không hoạt động"';
            return $res;
        }
