...
$treeID = 'new_category_tree_'.$model->id;
$buttonID = 'update_cate_new_'.$model->id;

if ($readOnly) {
    $js = <<<JS
    function handleNewCategoryUpdate{$treeID}(e){
        e.preventDefault();
...
JS;
    ...
    $this->registerJs("$('#{$buttonID}').click(handleNewCategoryUpdate$treeID);", \yii\web\View::POS_READY);
}
