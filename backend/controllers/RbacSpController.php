...
class RbacSpController extends \yii\web\Controller
{
...
//                'except' => ['index'],
    ...
    public function actionPermission()
    {
        $dataProvider = new ActiveDataProvider([
            'query' => AuthItem::findPermission()->andWhere(['acc_type' => AuthItem::ACC_TYPE_SP]),
        ]);
...
//            'searchModel' => $searchModel
    ...
    {
        $dataProvider = new ActiveDataProvider([
            'query' => AuthItem::findRole()->andWhere(['acc_type' => AuthItem::ACC_TYPE_SP]),
        ]);
...
//            'searchModel' => $searchModel
    ...
    public function actionCreatePermission()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            $model->acc_type = AuthItem::ACC_TYPE_SP;
    ...
    public function actionGeneratePermission()
    {
        $dataProvider = new ArrayDataProvider([
            'allModels' => AuthHelper::listActions('@site', true),
            'key' => 'name',
            'pagination' => [
                'pageSize' => 50,
            ],
...
//
    ...
    {
        $dataProvider = new ArrayDataProvider([
            'allModels' => AuthHelper::listControllers('@site', true),
            'key' => 'name',
            'pagination' => [
                'pageSize' => 50,
            ],
...
//
    ...
    public function actionGeneratePermissionConfirm(){
        ...
        if (isset($post['ids'])) {
            $ids = $post['ids'];
            $actions = AuthHelper::listActions('@site', true);

            $count = 0;
            foreach ($actions as $action){
                if ($action->isNameIn($ids) && $action->createPermission(AuthItem::ACC_TYPE_SP)){
                    $count++;
                }
            }
            return [
    ...
    public function actionGenerateRoleConfirm(){
        ...
        if (isset($post['ids'])) {
            $ids = $post['ids'];
            $controllers = AuthHelper::listControllers('@site', true);//FIXME: cuongvm 20150326 -  fixbug không tự add full. false->true

            $count = 0;
            foreach ($controllers as $controller){
                if ($controller->isNameIn($ids) && $controller->createRoleIfNotExist(AuthItem::ACC_TYPE_SP)){
                    $controller->autoPermissionAssign(AuthItem::ACC_TYPE_SP);
                    $count++;
                }
            }
            return [
